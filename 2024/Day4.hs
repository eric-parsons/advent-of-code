...
part2 rows = sum . map (fromEnum . isXmas) $ xGroups
  where
    ...
    xGroups = [concat g | rs <- triplets rows, g <- transpose . map triplets $ rs]
    isXmas g = g =~ "M.M.A.S.S|M.S.A.M.S|S.M.A.S.M|S.S.A.M.M" :: Bool
...
triplets :: [a] -> [[a]]
triplets (x1 : rest@(x2 : x3 : xs)) = [x1, x2, x3] : triplets rest
triplets _ = []
